...
Esta guía documenta el proceso estándar para agregar nuevos componentes a `@jotnar_dev/core`. Nace de las lecciones (varias, dolorosas) de los componentes `Button` e `Input` — cada regla acá abajo existe porque algo se rompió en silencio antes de tenerla.
...
- **Por qué esta regla es estricta y no una sugerencia:** el `Input` tuvo justo este bug — su estado `:disabled` usaba `var(--kamui-color-gray-400)` y `var(--kamui-color-gray-200)`, que no existían en la escala de grises (`100/300/500/700/900`). Nada en el pipeline lo detectó — ni `tsc`, ni `eslint`, ni `prettier`, ni `tsup`, ni el build de Storybook — porque una `var()` que apunta a una custom property indefinida no es un error de sintaxis, el navegador simplemente cae al valor heredado. Solo se nota mirando el componente renderizado. Antes de dar por buena la CSS de un componente nuevo, cruza cada `var(--kamui-...)` contra `tokens.css` generado (`pnpm --filter @jotnar_dev/core tokens:build` y revisa `src/tokens/tokens.css`), no contra lo que "debería" existir.
...
- Importa el componente directamente desde el paquete consumible: `import { <NombreComponente> } from '@jotnar_dev/core';` (nunca desde una ruta relativa a `src/` — así se prueba lo que un consumidor real va a instalar, no el código fuente).
...
| `build-storybook`  | La prueba real: si un consumidor externo (`@jotnar_dev/core`) puede resolver e importar el componente completo, con estilos | — este es el único paso que hubiera detectado tanto el bug de rutas de Fase 1 como el bug de tokens de Input                                   |
